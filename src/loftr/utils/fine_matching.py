...
class FineMatching(nn.Module):
    ...
    def forward(self, feat_0, feat_1, data:Dict[str, torch.Tensor]):
        """
        Args:
        ...
        """
        M, WW, C = feat_0.shape
        W = torch.tensor(int(math.sqrt(WW)))
        ...
        conf_matrix = softmax_matrix_f.reshape(m, -1)[:len(data['mconf']),...]
        val, idx = torch.max(conf_matrix, dim = -1)
        idx = idx[:,None]
