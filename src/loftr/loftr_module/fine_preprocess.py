...
class FinePreprocess(nn.Module):
    ...
    def inter_fpn(self, feat_c, x2, x1, stride: int):
    ...
    def forward(self, feat_c0, feat_c1, hw0_f: List[int], hw0_c:List[int], b_ids, hw0_i:List[int], hw1_i:List[int], 
        feats: List[torch.Tensor],
        # feats_x2_0, feats_x2_1, feats_x1_0, feats_x1_1, 
        i_ids, j_ids, hw1_c: List[int]):
        ...
        # # feats: 
        # # [feats_x2_0, feats_x1_0, feats_x2_1, feats_x1_1] 
        # x2_0, x2_1 = feats_x2_0, feats_x2_1 # 1/4 feat
        # x1_0, x1_1 = feats_x1_0, feats_x1_1 # 1/2 feat
        x2_0, x2_1 = feats[0], feats[2] # 1/4 feat
        x1_0, x1_1 = feats[1], feats[3] # 1/2 feat
        ...
        # feat_f0 = rearrange(feat_f0, 'n (c ww) l -> n l ww c', ww=W**2)
        # print("-------------------------------------------------------3.2-------------------------------------------------") 
        feat_f0 = torch.stack(feat_f0.split(int(W**2), dim=1)).permute(0, 3, 2, 1)
        
        feat_f1 = F.unfold(feat_f1, kernel_size=(W+2, W+2), stride=(stride, stride), padding=(1, 1))
        # feat_f1 = rearrange(feat_f1, 'n (c ww) l -> n l ww c', ww=(W+2)**2)
        feat_f1 = torch.stack(feat_f1.split(int((W+2)**2), dim=1)).permute(0, 3, 2, 1)
